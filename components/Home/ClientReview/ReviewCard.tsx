...
const ReviewCard = ({user}:Props) => {
  return (
    <div className="bg-white shadow-md dark:bg-gray-800 rounded-lg m-3 p-6 relative">
        <QuoteIcon className="w-8 h-8 abolute top-4 right-4 text-red-600 dark:text-yellow-300 opacity-20" />
        ...
        <div className="flex items-center space-x-4">
            <Image alt="reviewer-image" width={50} height={50} src={user.userImage} className="rounded-full"/>
            <div className="">
                <h1 className="font-bold text-gray-800 dark:text-gray-200">{user.name}</h1>
                <p className="text-sm  text-blue-500">{user.profession}</p>
            </div>
        </div>
    </div>
  )
